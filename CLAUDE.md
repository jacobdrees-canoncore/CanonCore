...
- Prefer Orca's tools: the browser (`orca tab`, `snapshot`, `click`, `fill`) over Playwright, and
  ...
  in progress**. All three answer `ok: true` (ADR-0162), and a parked agent's widget is sent
  nothing, not even navigation: its question is the Owner's to answer in that worktree (ADR-0187).
  **WAIT FOR THE PROMPT where it can wait**, and flush only where it cannot.
