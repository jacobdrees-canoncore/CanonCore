...
- Prefer Orca's tools: the browser (`orca tab`, `snapshot`, `click`, `fill`) over Playwright, and
  ...
  in progress**. Both answer `ok: true` (ADR-0162); ADR-0187 has the keystrokes per widget shape and
  what it eats meanwhile. **WAIT FOR THE PROMPT where it can wait**, and flush only where it cannot.
