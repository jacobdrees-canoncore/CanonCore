...
describe("one item, two orderings, two positions", () => {
  ...
  it.each([
    ...
    expect(
      item.placements.filter((placement) => placement.containerId === tmdbSeason2),
    ).toStrictEqual([]);
  });
});
...
describe("not a tree", () => {
  ...
  it("gives New Earth two parents, at two positions, from two sources", async () => {
    ...
    expect(item.placements).toHaveLength(2);
    expect(item.placements.map((placement) => placement.containerId).toSorted()).toStrictEqual(
      [wikiSeries2, tmdbSeason2].toSorted(),
    );
    // And the two rows are two rows rather than one read twice.
    expect(new Set(item.placements.map((placement) => placement.id)).size).toBe(2);
  });
...
function positionIn(
  ...
  const [only, ...rest] = item.placements.filter(
