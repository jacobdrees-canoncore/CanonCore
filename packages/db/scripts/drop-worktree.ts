 ...
 * each removed worktree had checked out (ADR-0191). `orca worktree rm` takes
 * the files and nothing else, and `db:setup`'s sweep spares anything younger
 * than an hour, which a worktree's test databases are when its PR merges.
 *
...
if (branches.length === 0) {
  console.error("usage: pnpm db:drop-worktree <branch>..., each one a removed worktree's branch.");
  process.exit(1);
}
