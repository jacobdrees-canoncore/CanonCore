...
-- single record and another as several parts (CNCORE-368's finding). That is
...
CREATE TABLE "part_disagreements" (
	"id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL,
	"owner_id" uuid NOT NULL,
	"item_id" uuid NOT NULL,
...
ALTER TABLE "part_disagreements" ADD CONSTRAINT "part_disagreements_owner_id_owners_id_fk" FOREIGN KEY ("owner_id") REFERENCES "public"."owners"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "part_disagreements" ADD CONSTRAINT "part_disagreements_item_id_items_id_fk" FOREIGN KEY ("item_id") REFERENCES "public"."items"("id") ON DELETE cascade ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "part_disagreements" ADD CONSTRAINT "part_disagreements_source_id_sources_id_fk" FOREIGN KEY ("source_id") REFERENCES "public"."sources"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
ALTER TABLE "part_disagreements" ADD CONSTRAINT "part_disagreements_merge_id_merges_id_fk" FOREIGN KEY ("merge_id") REFERENCES "public"."merges"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE UNIQUE INDEX "part_disagreements_one_per_source" ON "part_disagreements" USING btree ("item_id","source_id") WHERE "part_disagreements"."deleted_at" is null;--> statement-breakpoint
...
CREATE TRIGGER "part_disagreements_touch" BEFORE UPDATE ON "part_disagreements" FOR EACH ROW EXECUTE FUNCTION touch_row();
