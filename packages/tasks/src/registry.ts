...
import { boundedTo, unshowable } from "@canoncore/text";
...
function bounded(detail: string): string {
  return boundedTo(detail, BOUNDED_DETAIL) || UNSHOWABLE_DETAIL;
}
 ...
 *
 * `?? "It said nothing."` ON THAT PAGE DOES NOT CATCH IT, and would be the
 * wrong sentence if it did. That fallback tests for NULL and this is a non-null
 ...
 * `unshowable` AND NOT `quotedTo`. The other four callers interpolate a value
