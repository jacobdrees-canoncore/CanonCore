...
| PR merged | Done, except a ticket with THREE PRs, which stays In Review (ADR-0192) |
...
three-PR ticket above: the dispatcher reads its state back after the last merge, and sets `Done`
only if the automation did not.
