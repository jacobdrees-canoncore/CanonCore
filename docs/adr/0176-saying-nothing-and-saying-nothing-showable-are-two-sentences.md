...
> `packages/providers/src/reason.ts` asks `strippedToNothing` which of the two emptied a value and
...
> the phrase stays in one place. `strippedToNothing` is published from that leaf, where it was
...
The three sentences measure 71, 69 and 98 characters against `REASON_MAX_LENGTH`'s 300.

## The question moved to the leaf rather than the strip being spelled again

`strippedToNothing` asks whether `CONTROLS` removed anything. It was private because `quotedTo` was
...
spelling of a value in a Provider's voice. CNCORE-307 carries it; it is a different defect, because
...
**`failed()` in `client.ts` was read again and still owes these words nothing**, which is ADR-0179's
finding re-checked rather than taken on trust. A body of only controls gives `said === ""`, and the
sentence stops at `${answered}.` — so the Owner is told the status and not told there was a body.
That is an OMISSION and not a contradiction: nothing there asserts the Provider sent nothing, which
is the claim this record and ADR-0179 exist to remove. Left alone deliberately; changing it would be
a fourth site in a diff agreed for three.
