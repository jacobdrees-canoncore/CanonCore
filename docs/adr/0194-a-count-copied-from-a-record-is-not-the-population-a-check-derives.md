...
> figure: "the sixth and the seventh", "the seventh site" twice, "caught the sixth and not the
> seventh", "the eighth site" and "THE FIFTH SITE". Each now names the surface it meant. Nothing
> went into `tree-figures.test.ts`, because no argument rests on the figure, and no mechanism is
> added, as in [[0188-a-count-nobody-routes-by-is-deleted-rather-than-derived]]. No provider
...
once with its derived set of bounds emptied: `unboundedParameters(theSurfaces(), new Set())`, in a
row inside the file's `describe`. That names every surface and parameter whose value reaches a JSX
child position and is kept out of it ONLY by a bound, which is exactly the population the file
derives. The row lived in a scratch copy that was deleted afterwards, and the spec axis of this
ticket's review took it again independently, with the same three.
...
It was ADR-0163's five sites, as [[0170-a-value-a-page-both-asks-with-and-quotes-is-two-values]]
restates them with `?q=` at `/search` and `/import` as the sixth and seventh, copied in when
CNCORE-298 built the check. The five are `bounded` in `@canoncore/providers`, the repeat's refusal
in `@canoncore/db`, the overlong-id refusal in `@canoncore/api`, a task's detail in
`@canoncore/tasks`, and `?refused=` on `/settings`.
...
- **"FOUR of ADR-0163's five sites are in packages that render nothing"** stays, in the same
  docblock, along with its "The FIFTH is `?refused=` on `/settings`" and "ADR-0163's five sites" a
  paragraph above it. They count and index a record's closed list rather than the tree, and say so
  in the sentence, where the ordinals removed above named sites by their place in a list the
  docblock no longer states. The paragraph they sit in confesses that an earlier draft miscounted
  that same list, and the confession needs the number. The four packages it names are still where
  the sites outside this file's reach are, by a grep for the bounding calls at `4ecd698`.
