...
> fallback's stop, then gives this one's reason from its own slot. A whole sentence takes a stop; a
> name and a clause do not; this is a clause `failed` finishes. Its "the four sentences" is now "the
> fallbacks", by [[0188-a-count-nobody-routes-by-is-deleted-rather-than-derived]], and the
...
says why `UNNAMED` carries no stop: "a full stop there would read as part of the name", and
...
it the same way about a list: the number can go and the list can stay, because the list is evidence
...
- **Two direct callers of `unshowable` are left out of the docblock, deliberately.**
  `UNSHOWABLE_ENTRY` (`apps/web`) and `UNSHOWABLE_DETAIL` (`@canoncore/tasks`) are outside this
  package, and an example list is not a census.
