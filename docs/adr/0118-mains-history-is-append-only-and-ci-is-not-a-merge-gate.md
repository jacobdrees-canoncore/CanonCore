...
**The enforcement was real rather than merely stored.** While the full ruleset was active,
`GET /repos/.../branches/main` reported `protected: true` and the open pull request for this very
ticket moved to `mergeStateStatus: BLOCKED`. On teardown it returned to `CLEAN`. That is the
difference between an API that records a setting and a gate that stops a merge, and it is why the
measurement attempted a merge-blocking configuration rather than reading the endpoint.
...
on Free, that is false. The protected-branches page is the one that holds: *"You can enable branch
...
GitHub is four words: *"Avoid requiring workflows that can be skipped."* Three jobs in this
pipeline are matrix-named (`The image, built and run (linux/amd64, ubuntu-latest)`), so the list
cannot be transcribed from job names either.
...
ADR-0089, and a private repository in a GitHub Free organisation gets neither route: that is what
...
| `GET /repos/.../branches/main` while active | `protected: true` |
...
proves it exists. `gh api repos/jacobdrees-canoncore/CanonCore/rules/branches/main` is how a reader
checks this record against reality, and an empty answer means the ruleset was removed rather than
that this record was never true.
